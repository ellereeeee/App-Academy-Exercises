...
LITTLE_WORDS = [ "and", "the", "over", "a", "on", "of" ] # capitalized to make this a constant (and thus a global variable accessable by map() in titleize())

def titleize(title)
  words = title.split(" ")
  
  titleized_words = words.map.with_index do |word, i| # i and word were in the wrong order
